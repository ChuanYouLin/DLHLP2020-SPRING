...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    Examples::
        ...
        # print(logits.shape) (1, 55, 4) batch, t, class
        ...
        scores_up_to_t = torch.zeros(logits.shape[0], logits.shape[2])
        backpointer = torch.zeros(logits.shape)
        for t in range(logits.shape[1]):
            if t == 0:
                scores_up_to_t = logits[:, t, :] + torch.tensor([0, -float('inf'), -float('inf'), 0]).to(logits.device)
                backpointer[:, t, :] = torch.ones(logits.shape[0], logits.shape[2])
            else:
                scores_up_to_t, backpointer[:, t, :] = torch.max(
                    scores_up_to_t.unsqueeze(2) + logits[:, t, :].unsqueeze(1) + penalty.unsqueeze(0), 
                    dim=1
                )
        decoded = torch.zeros((logits.shape[0], logits.shape[1]), dtype=torch.long)
        decoded[:, -1] = torch.argmax(scores_up_to_t, dim=1)
        for t in list(reversed(range(1, logits.shape[1]))):
            decoded[:, t-1] = torch.cat([backpointer[x, t, y].unsqueeze(0) for x, y in enumerate(decoded[:, t])], dim=0)
            pointer = decoded[:, t]
        # print(decoded)
        # print(backpointer)
        # exit()
        new_logits = torch.zeros(logits.shape)
        for b in range(new_logits.shape[0]):
            for t in range(new_logits.shape[1]):
                new_logits[b, t, decoded[b, t]] = 1
        logits = new_logits
