...
class LocationAwareAttention(BaseAttention):
    ...
    def forward(self, q, k, v):
        ...
        # print('prev', self.prev_att.shape) [16, 1, 140]
        # print('loc_conv', self.loc_conv(self.prev_att).shape) [16, 10, 140]
        # print('loc_conv_trans', self.loc_conv(self.prev_att).transpose(1, 2).shape) [16, 140, 10]
        # print('loc_proj', self.loc_proj(self.loc_conv(self.prev_att).transpose(1, 2)).shape) [16, 140, 300]
        loc_context = torch.tanh(self.loc_proj(self.loc_conv(
            self.prev_att).transpose(1, 2)))  # BxNxT->BxTxD
        # print('cont', loc_context.shape) [16, 140, 300]
        # print('unsq', loc_context.unsqueeze(1).shape) [16, 1, 140, 300]
        # print('unsq_rep', loc_context.unsqueeze(1).repeat(1, self.num_head, 1, 1).shape) [16, 1, 140, 300]
        # print('view', loc_context.unsqueeze(1).repeat(1, self.num_head, 1, 1).view(-1, ts, self.dim).shape) [16, 140, 300]
        loc_context = loc_context.unsqueeze(1).repeat(
            1, self.num_head, 1, 1).view(-1, ts, self.dim)   # BxNxTxD -> BNxTxD
        q = q.unsqueeze(1)  # BNx1xD
        # print(k.shape, q.shape, loc_context.shape) torch.Size([1, 179, 300]) torch.Size([1, 1, 300]) torch.Size([1, 179, 300])
        # Compute energy and context
        energy = self.gen_energy(torch.tanh(
            k+q+loc_context)).squeeze(2)  # BNxTxD -> BNxT
        # print(energy.shape) torch.Size([1, 179])
        output, attn = self._attend(energy, v)
